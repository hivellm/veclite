...
//! (crates/vectorizer-core/src/quantization/binary.rs), Apache-2.0.
...
mod tests {
    ...
    fn test_binary_quantization_quantized_search() {
        ...
        let quantized_vector = quantizer
            .quantize_vector(&vectors[0])
            .unwrap_or_else(|e| panic!("{e}"));

        let similarity = quantizer
            .similarity(&query, &quantized_vector)
            .unwrap_or_else(|e| panic!("{e}"));
