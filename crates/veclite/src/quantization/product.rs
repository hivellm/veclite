...
//! (crates/vectorizer-core/src/quantization/product.rs), Apache-2.0.
...
//!
//! Additional adaptation beyond the standard trim/path localization: the
//! vendor source seeds K-means++ centroid initialization with
//! `fastrand::Rng`, which is not a VecLite workspace dependency (ADR-0001:
//! zero non-essential deps). This file vends the same K-means++
//! *algorithm* — random first centroid, then distance-weighted sampling for
//! the rest — but draws from a small dependency-free [`Rng`] defined below
//! instead of pulling in `fastrand`. Centroid initialization is
//! non-deterministic upstream too (`fastrand::Rng::new()` seeds from OS
//! entropy on every call), so this swap does not affect any byte-identity
//! guarantee: CORE-041 parity applies to the scalar/binary encoding math,
...
/// Minimal, dependency-free pseudo-random generator used only for K-means++
/// centroid initialization (see the module-level adaptation note). Not
/// cryptographic, not seedable/reproducible by design — matches the
/// upstream `fastrand::Rng::new()` call it replaces, which also seeds from
/// OS entropy on every invocation.
struct Rng(u64);

impl Rng {
    /// Seeds from a monotonically increasing counter mixed through
    /// `std::collections::hash_map::RandomState`'s per-process random key,
    /// so distinct `Rng::new()` calls diverge without touching a wall
    /// clock (`std::time::SystemTime::now()` panics on some `no_std`-ish
    /// targets; this only relies on `HashMap`'s already-required entropy
    /// source).
    fn new() -> Self {
    ...
    /// xorshift64* step — cheap, sufficient statistical quality for
    /// weighted-sample centroid selection.
    ...
    /// Uniform integer in `0..range.end`. Mirrors the `fastrand::Rng::usize`
    /// call shape used at the call site (`rng.usize(..subvectors.len())`).
    fn usize(&mut self, range: std::ops::RangeTo<usize>) -> usize {
        if range.end == 0 {
            0
        } else {
            (self.next_u64() % range.end as u64) as usize
        }
    }
...
mod tests {
    ...
    fn test_quantization_workflow() {
        ...
        let quantized = pq.quantize(&test_vector).unwrap_or_else(|e| panic!("{e}"));
        assert_eq!(quantized.len(), 4);

        // Test reconstruction
        let reconstructed = pq.reconstruct(&quantized).unwrap_or_else(|e| panic!("{e}"));
        assert_eq!(reconstructed.len(), 8);

        // Test quantization error
        let error = pq
            .quantization_error(&test_vector, &quantized)
    ...
    fn test_pq_compression_and_search_accuracy() {
        ...
        for original in test_vectors {
            let quantized = pq.quantize(original).unwrap_or_else(|e| panic!("{e}"));
            let reconstructed = pq.reconstruct(&quantized).unwrap_or_else(|e| panic!("{e}"));
