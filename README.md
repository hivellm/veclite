...
🚧 **Phase 1 complete; Phase 2 (persistence) complete — format v1 frozen at gate G2.** `VecLite::open(path)` opens a durable single-file database: the `.veclite` format v1 (4 KiB header, immutable CRC'd segments, MessagePack TOC, root-pointer-swap commit — [SPEC-002](docs/specs/SPEC-002-storage-format.md)) plus a write-ahead log ([SPEC-003](docs/specs/SPEC-003-wal-durability.md)) with three durability modes, checkpointing, and crash recovery — kill-9 never corrupts the file. The crash suite proves it: randomized workloads with an oracle model, torn-write and bit-flip sweeps, and a real subprocess kill-9 harness (`cargo xtask crash`, 10 000 iterations nightly on Linux/macOS/Windows), with committed v1 golden files guarding the freeze. **SPEC-002/SPEC-003 are frozen-normative** — the byte format is fixed (phase 2e). Search now takes payload filters (phase 3a): a Qdrant-style `Filter { must, should, must_not }` with server-parity semantics, built in Rust or parsed from portable JSON, applied via `query(v).filter(f)`; declared `Keyword`/`Integer`/`Float` payload indexes accelerate filtered queries (results identical to a scan), can be added at runtime with `create_payload_index` (journaled + sealed as PIDX segments, surviving crash and reopen), and a selectivity planner picks between exact pre-filter and HNSW over-fetch post-filter with an exact fallback (phase 3e). Auto-embed collections (phase 3b) turn text into vectors offline with pure-Rust sparse providers (`bm25` default, plus `tfidf`/`bow`/`char_ngram`, and `svd` behind the `svd` feature): `upsert_text`/`search_text` embed with the collection's provider, the vocabulary updates incrementally and persists in the file (phase 3f) — reopen searches identically with no rebuild — custom providers plug in per database via `register_embedder`, provider outputs are pinned to the server within 1e-5 by a generated parity corpus, and a UTF-8-safe `chunk::Chunker` splits long text. `hybrid_query()` fuses a dense and a sparse lane with deterministic reciprocal rank fusion (phase 3c), `alpha`/`rrf_k` tunable; a single lane degenerates to plain search. The sparse lane persists as a SPARSE segment surviving reopen and crash recovery, `hybrid_query().text(q)` fills both lanes from one string on auto-embed collections, and a committed conformance corpus pins the fused rankings (phase 3g). The API surface is rounding out (phase 3d): collection aliases for blue-green swaps, cursor-based `scroll`, parallel `search_batch`, and `stats()`. Bindings are underway: a panic-safe C ABI (`veclite-ffi`, phase 4a) and a PyO3 Python binding with NumPy zero-copy and GIL release (`veclite-py`, phase 4b, `pip install`-able abi3 wheels). Opening takes an advisory lock (exclusive read-write, shared read-only) so a second opener fails fast with `Locked` instead of corrupting; `OpenOptions::read_only(true)` serves reads and searches while rejecting writes with `ReadOnly` (phase 2c). `db.snapshot(path)` writes a standalone compacted copy without blocking writers, and `db.vacuum()` reclaims dead space in place — shrinking the file after bulk deletes and escalating automatically past the tombstone threshold (phase 2d). Memory-mapped reads landed in phase 2f ([ADR-0004](.rulebook/decisions/004-single-file-mmap-vectors-with-exact-brute-force-larger-than-ram-tier.md)): big collections keep their vectors in a read-only map of the same single file — under `OpenOptions::memory_budget` the HNSW graph is rebuilt from the map on open, past it searches run as exact SIMD scans over the map, so datasets larger than RAM open and serve with exact recall; unchanged mapped collections checkpoint by reference (nothing rewritten), and the v1 byte format is untouched.
