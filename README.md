...
🚧 **Phase 1 complete; Phase 2 (persistence) started.** The `.veclite` on-disk format v1 — 4 KiB header, immutable CRC'd segment codec, MessagePack TOC, and the crash-safe root-pointer-swap commit protocol — is implemented and property/fuzz/crash tested ([SPEC-002](docs/specs/SPEC-002-storage-format.md)); wiring it into `VecLite::open` (WAL, mmap) comes next.
